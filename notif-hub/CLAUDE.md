...
モック通知が3件表示され、
詳細画面で「明日中に共有予定と返信」と入力 →「生成」で返信文が出れば成功。

## 完了の定義（Phase 1）

- [ ] フローティングウィンドウが画面右下に出る
- [ ] モック通知3件がリスト表示される
- [ ] 通知クリックで詳細画面に遷移する
- [ ] 「指示」入力 → 「生成」で Agent SDK 経由で返信文が表示される
- [ ] 「送信」ボタンを押すと console.log() に内容が出る（実送信は Phase 2）

## Phase 2 以降

- Phase 2: Gmail MCP 接続（最も実装が単純なため最初）
- Phase 3: Slack / Notion / Figma を順次追加
- Phase 4: 自動ポーリング + macOSネイティブ通知
